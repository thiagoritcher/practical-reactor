...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void behold_flatmap() {
        ...
        Mono<String> currentUserEmail = getCurrentUser().flatMap(u -> getUserEmail(u));
    ...
    public void streaming_service() {
        ...
        streamingService().flatMapMany(m -> m);
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((s, t) -> 
        	"span".equals(s.get().metaData) ? t:  mailBoxSecondary());
    ...
    public void instant_search() {
        //todo: feel free to change code as you need
        Flux<String> suggestions = userSearchInput().switchMap(m -> autoComplete(m));
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
        		.concatMap(t -> t.flatMap(s -> commitTask(s).thenReturn(s)));
...
//        BlockHound.install(); //don't change this line, blocking = cheating!
        ...
        Flux<String> stream = Flux.usingWhen(
				...
				n -> n,
				r -> StreamingConnection.closeConnection());
